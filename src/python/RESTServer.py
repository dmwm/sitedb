...
def filter(match, cursor):
  ...
  and the `match` predicate can be invoked with ``match(*row)`` for each
  retrieved row. If `match` returns True, the original row is included in
  the generator output, otherwise it's skipped.

  :arg callable match: Predicate.
