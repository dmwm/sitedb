...
def create_submodule_file(master_package, subroot, package, py_file, opts):
    ...
    #print "*** CREATE_PACKAGE_FILE %s is_package: %s file: %s" % (package, is_package, py_file)
    if is_package:
        ...
        return ('', create_module_file(package, py_file, opts))
...
def create_package_file(root, master_package, subroot, py_files, opts, subs):
    ...
    if subs:
        tocpart += '\n'
        tocpart += format_heading(2, 'Subpackages')
        tocpart += '.. toctree::\n\n'
        for sub in subs:
