...
def define_the_build(self, dist, system_name, patch_x = ''):
  ...
  dist.py_modules = ['SiteDB.__init__']
...
class BuildCommand(Command):
  ...
  description = \
    ...
    "\t\t   at the moment is 'SiteDB'. Use with --force to ensure a clean\n" + \
  ...
  def finalize_options(self):
    if self.system not in systems:
      print "System %s unrecognised, please use '-s SiteDB'" % self.system
      sys.exit(1)
...
class InstallCommand(install):
  ...
  description = \
    "Install a specific system, either 'SiteDB'. You can patch an existing\n" + \
